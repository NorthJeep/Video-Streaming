...
if(!defined('INDEX_VIEW')){
  die("<strong>403:</strong> Direct Access Forbidden!");
	...
	$query = mysqli_query($connect, "SELECT `bg_color` FROM `background` where template_id = 4");
...
$query = mysqli_query($connect, "select * from upload where template_id = 4");
while ($row = mysqli_fetch_row($query)){
	if($row[3] == 'header-left'){
		$img_left = '<img src="upload/template_4/images/'.$row[1].'" style="max-width:100%; max-height:100%;">';
	}
	else if($row[3] == 'header-mid'){
		$img_middle = '<img src="upload/template_4/images/'.$row[1].'" style="max-width:100%; max-height:100%;">';
	}
	else if($row[3] == 'header-right'){
		$img_right = '<img src="upload/template_4/images/'.$row[1].'" style="max-width:100%; max-height:100%;">';
	}
}
...
<div class="main-container">
	<div class="main-left">
		<?php
			...
			$stat_m = getVolume('main-left', $connect, 4);

			if(!empty($name)){
				echo "
					<video class='videoInsert' id='main_left' width='100%' height='100%' autoplay muted preload>
					<source src='upload/template_4/videos/".$name[0]['name']."'>
					</video>
		...
		?>
			...
			videoPlayer2.onended = function run(){
				...
				var nextVideo = "upload/template_4/videos/"+list[video_count].name;
	...
	<div class="main-right">
		<?php
			...
			$stat_m2 = getVolume('main-right', $connect, 4);
		...
		?>
			...
			videoPlayer4.onended = function run(){
				...
				var nextVideo = "upload/template_4/videos/"+list2[video_count2].name;
...
<div class="footer-container">
	...
	$query_marquee = mysqli_query($connect, "SELECT * FROM marquee WHERE template_id = 4");
