...
if(!defined('INDEX_VIEW')){
  die("<strong>403:</strong> Direct Access Forbidden!");
	...
	$query = mysqli_query($connect, "SELECT `bg_color` FROM `background` where template_id = 3");
...
$query = mysqli_query($connect, "select * from upload where template_id = 3");
while ($row = mysqli_fetch_row($query)){
	if($row[3] == 'header-left'){
		$img_left = '<img src="upload/template_3/images/'.$row[1].'" style="max-width:100%; max-height:100%;">';
	}
	else if($row[3] == 'header-mid'){
		$img_middle = '<img src="upload/template_3/images/'.$row[1].'" style="max-width:100%; max-height:100%;">';
	}
	else if($row[3] == 'header-right'){
		$img_right = '<img src="upload/template_3/images/'.$row[1].'" style="max-width:100%; max-height:100%;">';
	}
}
				...
				<div id='slider'>
					<?php
						...
						while($row = mysqli_fetch_array($query)){
							echo "<img src='upload/template_3/images/".$row['name']."'>";
						}
	...
	<div class="main-second">
		<div class="main-left">
			<?php
				...
				$stat_m = getVolume('second-left', $connect, 3);

				if(!empty($name)){
					echo "
						<video class='videoInsert' id='main_left' width='100%' height='100%' autoplay muted preload>
						<source src='upload/template_3/videos/".$name[0]['name']."'>
						</video>
			...
			?>
				...
				videoPlayer2.onended = function run(){
					...
					var nextVideo = "upload/template_3/videos/"+list[video_count].name;
		...
		<div class="main-right">
			<?php
				$name = array();
				$query = mysqli_query($connect, "select * from upload where location = 'second-right' and template_id = 3 and status = 1");
				$video_count = 0;
				while($row = mysqli_fetch_array($query)){
					$name[] = array("video" => $video_count,"id" => $row['id'], "name" => $row['name']);
					$video_count++;
				...
				$stat_m2 = getVolume('second-right', $connect, 3);
			...
			?>
				...
				videoPlayer4.onended = function run(){
					...
					var nextVideo = "upload/template_3/videos/"+list2[video_count2].name;
...
<div class="footer-container">
	...
	$query_marquee = mysqli_query($connect, "SELECT * FROM marquee WHERE template_id = 3");
