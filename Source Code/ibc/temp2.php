...
if(!defined('INDEX_VIEW')){
  die("<strong>403:</strong> Direct Access Forbidden!");
	...
	$query = mysqli_query($connect, "SELECT `bg_color` FROM `background` where template_id = 2");
...
$query = mysqli_query($connect, "select * from upload where template_id = 2");
while ($row = mysqli_fetch_row($query)){
	if($row[3] == 'header-left'){
		$img_left = '<img src="upload/template_2/images/'.$row[1].'" style="max-width:100%; max-height:100%;">';
	}
	else if($row[3] == 'header-mid'){
		$img_middle = '<img src="upload/template_2/images/'.$row[1].'" style="max-width:100%; max-height:100%;">';
	}
	else if($row[3] == 'header-right'){
		$img_right = '<img src="upload/template_2/images/'.$row[1].'" style="max-width:100%; max-height:100%;">';
	}
}
...
<div class="main-container">
	<div class="main-mainad">
		<?php
			...
			$stat_m = getVolume('main-ad', $connect, 2);

			if(!empty($name)){
				echo "
					<video class='videoInsert' id='main_ad' width='100%' height='100%' autoplay muted preload>
					<source src='upload/template_2/videos/".$name[0]['name']."'>
					</video>
		...
		?>
			...
			videoPlayer2.onended = function run(){
				...
				var nextVideo = "upload/template_2/videos/"+list[video_count].name;
	...
	<div class="main-second">
		<div class="main-secondtop">
			<?php
				...
				$stat_m2 = getVolume('second-top', $connect, 2);
			...
			?>
				...
				videoPlayer4.onended = function run(){
					...
					var nextVideo = "upload/template_2/videos/"+list2[video_count2].name;
				...
				<div id="container">
					<div id='slider'>
						<?php
							...
							while($row = mysqli_fetch_array($query)){
								echo "<img src='upload/template_2/images/".$row['name']."'>";
							}
...
<div class="footer-container">
	...
	$query_marquee = mysqli_query($connect, "SELECT * FROM marquee WHERE template_id = 2");
